...
class CotizacionViewSet(viewsets.ModelViewSet):
    ...
    def promediar_periodo(self, request):
        ...
        if fecha_inicio and fecha_fin:
            ...
            return Response({'error': 'Ingrese los parámetros requeridos'}, status=400)
    ...
    def create(self, request):
        ...
        if fecha and monto:
            ...
            if cotizacion:
                ...
                print("Nueva compra ingresada con exito.")
                return Response(serializer.data, status=status.HTTP_201_CREATED, headers=headers)
            else:
                return Response({'error':'No hay cotización disponible '}, status=status.HTTP_400_BAD_REQUEST)
        else:
            print(" ingresa nueva fecha y monto .")
            return Response({'error':' ingrese nueva fecha y  nuevo monto '}, status=status.HTTP_400_BAD_REQUEST)